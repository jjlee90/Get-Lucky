...
const dealerImg = document.getElementById("dealer").append(createImg("bj", "dealer"))

// getting player id and appending image
const playerImg = document.getElementById("player").append(createImg("bj", "player"))
...
class player {
    ...
    checkAce(playerTotal) {
        if (this.cardArr[1].includes("ace") && this.aceCount > 0 && this.total > 21) {
            this.aceCount -= 1
            this.total -= 10
            playerTotal.innerHTML = this.total
        ...
        if (this.total > 21) {
            for (var i = 0; i < this.cardArr.length; i++) {
                if (this.cardArr[i].includes("ace") && this.aceCount > 0 && this.total > 21) {
...
function removeReset() { document.body.removeEventListener('click', reset) }

function checkForBlackJack() {
    //if both playerTotal and dealerArr = 21, its a push. (tie)
    if (player1.total == 21 && dealer.cardArr[0][2] + dealer.cardArr[1][2] == 21) {
    ...
    } else if (player1.total == 21 && dealer.cardArr[0][2] + dealer.cardArr[1][2] != 21) {
    ...
    } else if (dealer.cardArr[0][2] + dealer.cardArr[1][2] == 21 && player1.total != 21) {
...
function handResult() {
    if (player1.total > 21) {
        ...
        bankRoll.innerHTML = parseInt(bankRoll.innerHTML) + ((parseInt(wagerArea.innerHTML) + parseInt(player1.doubleDownBet)) * 2)
        ...
        bankRoll.innerHTML = parseInt(bankRoll.innerHTML) + (parseInt(wagerArea.innerHTML) + parseInt(player1.doubleDownBet))
...
twentyFiveBtn.addEventListener('click', () => { removeReset(), player1.bet = true, betAmount(25) })
hundredBtn.addEventListener('click', () => { removeReset(), player1.bet = true, betAmount(100) })
fiveHundredBtn.addEventListener('click', () => { removeReset(), player1.bet = true, betAmount(500) })
...
function bet() {
    ...
    if (player1.bet === false) {
        ...
        setTimeout(() => { startGame(), player1.start = true }, 25)
    }
